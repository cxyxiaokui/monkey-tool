...
import cn.zqmy.monkeytool.apiversion.annotation.TerminalVersion;
...
public class ClientVersionRequestCondition extends AbstractRequestCondition<ClientVersionRequestCondition> {
    ...
    private static Set<TerminalVersionExpression> parseByExpression(String[] stringExpressions) {
        ...
        for (String expression : stringExpressions) {
            ...
            while (matcher.find()) {
                ...
                String version = "";
                VersionOperatorEnum operator = VersionOperatorEnum.NIL;
                for (int i = 1; i <= matcher.groupCount(); i++) {
                    String content = matcher.group(i);
                    if (i == 1) {
                        if (StringUtils.isNotBlank(content) && !content.equalsIgnoreCase(str)) {
                        ...
                        operator = VersionOperatorEnum.parse(content);
                        if (operator == null) {
    ...
    static class TerminalVersionExpression {
        ...
        private VersionOperatorEnum operator;
        ...
        public TerminalVersionExpression(int[] terminals, String version, VersionOperatorEnum operator) {
            Arrays.sort(terminals);
            if (StringUtils.isNotBlank(version) && operator == VersionOperatorEnum.NIL) {
        ...
        public final boolean match(HttpServletRequest request) {
            ...
            if (this.operator != null && this.operator != VersionOperatorEnum.NIL) {
